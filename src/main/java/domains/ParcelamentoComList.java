...
import domains.Parcela;

public class ParcelamentoComList {
    ...
    private static void validarQuantidadeDeParcelas(int quantidadeDeParcelas) {
        if (quantidadeDeParcelas <= 0) {
            throw new RuntimeException("O número de parcelas deve ser maior que zero!");
        }
    }

    private static void validarValorAParcelar(BigDecimal valor) {
        if (valor.scale() != 2) {
            throw new RuntimeException("O valor deve ter duas casas decimais!");
        }
